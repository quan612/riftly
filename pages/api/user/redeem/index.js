...
const riftlyRedeemInteract = async (req, res) => {
  ...
  switch (method) {
    case "POST":
      try {
        const { OPERATION_WALLET_PRIVATE_KEY, NEXT_PUBLIC_INFURA_ID, INFURA_SECRET } =
          process.env;
        ...
        if (wallet.length < 16 || !utils.getAddress(wallet)) {
          return res.status(200).json({ message: "Wallet not linked or invalid wallet address", isError: true });
        }
        ...
        const signerWallet = new ethers.Wallet(
          `0x${OPERATION_WALLET_PRIVATE_KEY}`,
          infuraProvider
        );
