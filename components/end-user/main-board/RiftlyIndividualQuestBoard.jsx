...
const REDEEM = 2;
const ACHIEVEMENT = 3;
const PERSONAL_INFO = 4;

const RiftlyIndividualQuestBoard = ({ session }) => {
    const [menu, menuSet] = useState(DASHBOARD);
                        ...
                        <AnimatePresence initial={false}>
                            ...
                            >
                                {menu === DASHBOARD && (
                                    <>
                                        <FeatureQuests key={"2"} />
                                        <ChallengeQuests key="challenges-section" />
                                    </>
                                )}

                                {menu === REDEEM && <RedeemRewards key="redeem" />}
                    ...
                    <Flex h="100%" w="80%">
                        ...
                        >
                            <Box
                                ...
                                onClick={() => menuSet(DASHBOARD)}
                            >
                                <HomeMenuIcon fill={menu === DASHBOARD ? "#00BBC7" : "#1D3148"} />
                            </Box>
                            <Box
                                boxSize={"24px"}
                                _hover={{ cursor: "pointer" }}
                                onClick={() => menuSet(REDEEM)}
                            >
                                <RedeemMenuIcon fill={menu === REDEEM ? "#00BBC7" : "#1D3148"} />
                            </Box>
                            <Box
                                boxSize={"24px"}
                                _hover={{ cursor: "pointer" }}
                                onClick={() => menuSet(ACHIEVEMENT)}
                            >
                                <AchievementMenuIcon
                                    fill={menu === ACHIEVEMENT ? "#00BBC7" : "#1D3148"}
                                />
                            </Box>
                            <Box
                                boxSize={"24px"}
                                _hover={{ cursor: "pointer" }}
                                onClick={() => menuSet(PERSONAL_INFO)}
                            >
                                <PersonalMenuIcon
                                    fill={menu === PERSONAL_INFO ? "#00BBC7" : "#1D3148"}
                                />
                            </Box>
                        </Flex>
                    </Flex>
