  ...
  switch (method) {
    case 'GET':
      try {
        ...
        if (!eventName) {
          return res.status(200).json({ isError: true, message: 'Missing eventName query.' })
        }
        ...
        let currentImageQuest = allImageQuests.find(
          (q) => q.extendedQuestData.eventName.toLowerCase() === eventName.toLowerCase(),
        )
