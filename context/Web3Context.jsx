...
export function Web3Provider({ session, children }) {
  ...
  const adminSignIn = async (walletType) => {
    if (!walletType) {
      throw new Error(
        "Missing type of wallet when trying to setup wallet provider"
      );
    }
    ...
    try {
      ...
      if (!admin) {
        throw new Error(
          "Cannot authenticate as admin with current wallet account"
        );
      }
...
const removeLocalStorageWalletConnect = () => {
  ...
  const walletMobileCache = localStorage.getItem(
